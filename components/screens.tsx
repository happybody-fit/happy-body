...
import type { BodyState, EquipmentId, Exercise, GoalId, HappyBodyData, Pathway, PathwayId, PracticeEntry, ProgressionLevel, Recommendation, ScreenId, UserProfile } from '@/lib/types';
  ...
  return (
    <section className="body-map-section">
      ...
      <div className="pathway-progress-grid">{categoryPathways.map((pathway) => {
        ...
        const primaryBranch = pathway.branches?.find((branch) => branch.primary) ?? pathway.branches?.[0];
        const primaryLevelId = primaryBranch ? state?.branchLevels?.[primaryBranch.id] ?? current?.id : current?.id;
        const primaryStep = primaryBranch && primaryLevelId ? primaryBranch.levelIds.indexOf(primaryLevelId) : state?.currentLevel ?? -1;
        const nextLevelId = primaryBranch ? primaryBranch.levelIds[primaryStep + 1] : pathway.levels[primaryStep + 1]?.id;
        const next = nextLevelId ? pathway.levels.find((level) => level.id === nextLevelId) ?? null : null;
        const branchRows = pathway.branches?.map((branch) => {
          const savedId = state?.branchLevels?.[branch.id] ?? (branch.primary ? current?.id : undefined);
          return { branch, level: savedId ? pathway.levels.find((level) => level.id === savedId) ?? null : null };
        });
        const painPaused = state?.status === 'pain-flagged';
        const knownBranches = branchRows?.filter((item) => item.level).length ?? 0;
        const statusLabel = painPaused ? 'Paused after pain' : branchRows ? `${knownBranches} of ${branchRows.length} placed` : current ? 'Current step saved' : 'Not checked yet';
        return <article className={`path-progress-card compact ${pathway.tone} ${current ? 'known' : 'unknown'}`} key={pathway.id}>
          ...
          {branchRows ? <div className="body-map-branch-list">{branchRows.map(({ branch, level }) => <div key={branch.id}><small>{branch.title}</small><strong>{level?.title ?? 'Not placed yet'}</strong></div>)}</div> : current && <><p className="current-capability"><small>CURRENT STEP</small><strong>{current.title}</strong></p><p className="next-capability">{next ? <>Next: <strong>{next.title}</strong></> : <>Destination reached: <strong>{pathway.destination}</strong></>}</p></>}
...
function PathwayLadderStep({ level, pathway, stepNumber, current, done, next, openDetail, openPractice }: { level: ProgressionLevel; pathway: Pathway; stepNumber: number; current: boolean; done: boolean; next: boolean; openDetail: (exercise: Exercise) => void; openPractice: (item: Recommendation | null) => void }) {
  const item = level.exercises[0];
  return <article className={`ladder-step ${current ? 'current' : ''} ${done ? 'done' : ''}`}>
    <span className="step-index">{done ? '✓' : stepNumber}</span>
    <div className="ladder-main"><button className="ladder-open-movement" onClick={() => openDetail(item)} aria-label={`Open ${item.title} guidance`}><span><small>{current ? 'YOUR CURRENT STEP' : next ? 'NEXT VISIBLE STEP' : `STEP ${stepNumber}`}</small><strong>{level.title}</strong><span>{level.description}</span></span><i aria-hidden="true">→</i></button><details><summary>Quick guidance</summary><div><p><strong>{item.title}</strong> · {item.sets} sets · {item.reps ?? item.hold}</p><ul>{item.cues.map((cue) => <li key={cue}>{cue}</li>)}</ul><button className="text-button" onClick={() => openPractice({ id: `explore-${item.id}`, kind: 'practice', movementId: pathway.id, pathwayId: pathway.id, title: item.title, reason: 'You chose this from the full pathway.', detail: item.purpose, minutes: item.durationMinutes, score: 0, exerciseId: item.id })}>Practise or record this →</button></div></details></div>
    <span className="milestone-copy">{level.milestone}</span>
  </article>;
}
...
export function ExploreScreen({ data, initialPathwayId, updateProfile, startAssessment, openPractice }: { data: HappyBodyData; initialPathwayId: PathwayId; updateProfile: (profile: UserProfile) => void; startAssessment: (id: PathwayId) => void; openPractice: (item: Recommendation | null) => void }) {
  ...
  return (
    <>
      ...
      <div className="explore-tabs" role="tablist" aria-label={`${category} pathways`}>{categoryPathways.map((item) => <button role="tab" aria-selected={activeId === item.id} className={activeId === item.id ? 'active' : ''} key={item.id} onClick={() => setActiveId(item.id)}><span>{item.symbol}</span><strong>{item.name}</strong><small>{item.branches?.length ? `${item.branches.length} branches` : `${item.levels.length} steps`}</small></button>)}</div>
      ...
      <section className={`pathway-ladder ${pathway.branches?.length ? 'branched' : ''}`}><div className="section-heading"><div><p className="eyebrow">{pathway.branches?.length ? 'CHOOSE A SKILL BRANCH' : 'THE FIRST-PASS PATHWAY'}</p><h2>{pathway.branches?.length ? 'One pathway can grow in more than one direction.' : `From an accessible beginning to ${pathway.destination.toLowerCase()}`}</h2></div></div>{pathway.branches?.length ? <div className="pathway-branches">{pathway.branches.map((branch) => { const branchLevels = branch.levelIds.map((id) => pathway.levels.find((level) => level.id === id)).filter((level): level is ProgressionLevel => Boolean(level)); const savedLevelId = state?.branchLevels?.[branch.id] ?? (branch.primary && state?.currentLevel !== null && state?.currentLevel !== undefined ? pathway.levels[state.currentLevel]?.id : undefined); const currentBranchIndex = savedLevelId ? branch.levelIds.indexOf(savedLevelId) : -1; return <section className="pathway-branch" key={branch.id}><header><div><p className="eyebrow">{branch.primary ? 'FOUNDATION BRANCH' : 'SKILL BRANCH'}</p><h3>{branch.title}</h3><p>{branch.description}</p></div><span>{branchLevels.length} steps</span></header>{branchLevels.map((level, index) => <PathwayLadderStep key={level.id} level={level} pathway={pathway} stepNumber={index + 1} current={currentBranchIndex === index} done={currentBranchIndex > index} next={currentBranchIndex + 1 === index} openDetail={setDetailExercise} openPractice={openPractice} />)}</section>; })}</div> : pathway.levels.map((level, index) => <PathwayLadderStep key={level.id} level={level} pathway={pathway} stepNumber={index + 1} current={state?.currentLevel === index} done={(state?.currentLevel ?? -1) > index} next={(state?.currentLevel ?? -1) + 1 === index} openDetail={setDetailExercise} openPractice={openPractice} />)}</section>
      <PainNote />
      {detailExercise && <ExerciseDetailDialog exercise={detailExercise} pathwayName={pathway.name} close={() => setDetailExercise(null)} practice={() => { const item = detailExercise; setDetailExercise(null); openPractice({ id: `detail-${item.id}`, kind: 'practice', movementId: pathway.id, pathwayId: pathway.id, title: item.title, reason: 'You chose this from the movement guidance.', detail: item.purpose, minutes: item.durationMinutes, score: 0, exerciseId: item.id }); }} />}
    </>
  );
}

function ExerciseDetailDialog({ exercise, pathwayName, close, practice }: { exercise: Exercise; pathwayName: string; close: () => void; practice: () => void }) {
  ...
  return (
    <div className="dialog-backdrop exercise-video-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) close(); }}>
      <section className="practice-dialog exercise-video-dialog" role="dialog" aria-modal="true" aria-labelledby="exercise-video-title">
        ...
        {exercise.videoId && <div className="exercise-video-frame"><iframe src={`https://www.youtube-nocookie.com/embed/${exercise.videoId}?playsinline=1&rel=0`} title={`${exercise.title} demonstration`} referrerPolicy="strict-origin-when-cross-origin" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowFullScreen /></div>}
        <div className="exercise-detail-body">
          {exercise.videoId && exercise.videoStatus === 'placeholder' && <div className="temporary-video-note"><span>Temporary YouTube demonstration</span>{source && <a href={source.watchUrl} target="_blank" rel="noreferrer">Source: {source.sourceLabel} ↗</a>}</div>}
...
export function DiaryScreen({ data, openPractice }: { data: HappyBodyData; openPractice: (item: Recommendation | null) => void }) {
  ...
  const events = useMemo(() => [
    ...data.practices.map((item) => ({ id: item.id, type: 'practice' as const, date: item.date, title: item.exerciseTitle, detail: `${item.completion} · ${item.difficulty} · body ${item.bodyAfter}`, note: item.notes, pathwayId: item.pathwayId, author: item.recordedBy?.kind === 'coach' ? `Added by ${item.recordedBy.coachName}` : null })),
    ...data.assessments.map((item) => ({ id: item.id, type: 'assessment' as const, date: item.date, title: item.checkpointTitle, detail: item.outcome.replace('-', ' '), note: item.note, pathwayId: item.pathwayId, author: null })),
  ...
  return (
    <>
      ...
      <section className="diary-timeline">{events.length ? events.map((event) => <article key={`${event.type}-${event.id}`}><time>{formatDate(event.date)}</time><span className={`event-icon ${event.type}`}>{event.type === 'practice' ? '↟' : '◎'}</span><div><small>{event.type}{event.pathwayId ? ` · ${pathwayById[event.pathwayId].name}` : ''}{event.author ? ` · ${event.author}` : ''}</small><h3>{event.title}</h3><p>{event.detail}</p>{event.note && <blockquote>{event.note}</blockquote>}</div></article>) : <div className="diary-empty"><span>○</span><h2>Your diary begins with noticing.</h2><p>Record something you tried, even if you only did part of it.</p><button className="secondary-button" onClick={() => openPractice(null)}>Record my first practice</button></div>}</section>
    </>
  );
}
