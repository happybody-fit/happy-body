  ...
  return (
    <div className="dialog-backdrop practice-focus" role="presentation" onMouseDown={(event) => { if (event.currentTarget === event.target) close(); }}>
      <section className="practice-dialog practice-guide" role="dialog" aria-modal="true" aria-labelledby="practice-title">
        ...
        {item.videoId && <details className="video-details"><summary>Watch demonstration <span>{item.videoStatus === 'approved' ? 'Happy Body video' : 'Prototype video'}</span></summary><div className={`video-wrap ${item.videoStatus === 'approved' ? 'portrait' : ''}`}><iframe src={`https://www.youtube-nocookie.com/embed/${item.videoId}`} title={`${item.title} demonstration`} loading="lazy" referrerPolicy="strict-origin-when-cross-origin" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowFullScreen /></div>{item.videoStatus === 'placeholder' && <p>This public demonstration is a temporary placeholder. Happy Body-owned teaching will replace it before a broad content launch.</p>}</details>}
